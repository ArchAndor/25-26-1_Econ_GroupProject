...
CombinedData$Motorway <- ifelse(grepl("M0", CombinedData$Address, ignore.case = TRUE), 1, 
                                ifelse(grepl("M1", CombinedData$Address, ignore.case = TRUE), 1, 
                                       ifelse(grepl("M2", CombinedData$Address, ignore.case = TRUE), 1, 
                                              ifelse(grepl("M3", CombinedData$Address, ignore.case = TRUE), 1, 
                                                     ifelse(grepl("M4", CombinedData$Address, ignore.case = TRUE), 1, 
                                                            ifelse(grepl("M5", CombinedData$Address, ignore.case = TRUE), 1, 
                                                                   ifelse(grepl("M6", CombinedData$Address, ignore.case = TRUE), 1, 
                                                                          ifelse(grepl("M7", CombinedData$Address, ignore.case = TRUE), 1, 0))))))))




#model5+motorway
model_gas_9 = lm(Gasoline_average ~ Company_25cut + Locality_simple + People_per_household + Stations_per_1000 + Motorway, data = CombinedData)
summary(model_gas_9)

#model7+motorway 
model_gas_10 = lm(Gasoline_average ~ Company_25cut + Locality_simple + People_per_household + Stations_per_1000 + Cars_per_1000 + Motorway, data = CombinedData)
summary(model_gas_10)

#model9-people per household
model_gas_11 = lm(Gasoline_average ~ Company_25cut + Locality_simple + Stations_per_1000 + Motorway, data = CombinedData)
summary(model_gas_11)

#model10-people per household
model_gas_12 = lm(Gasoline_average ~ Company_25cut + Locality_simple + Stations_per_1000 + Cars_per_1000 + Motorway, data = CombinedData)
summary(model_gas_12)
#cars per 1000 still not significant, probably the best model is either model 9 or model 11

AIC(model_gas_9, model_gas_11)
BIC(model_gas_9, model_gas_11)

#another model where I added wage and deleted stations per 1000 
model_gas_13 = lm(Gasoline_average ~ Company_25cut + Locality_simple + Motorway + Wage, data = CombinedData)
summary(model_gas_13)
#wage now might be significant - worth considering this model as well
